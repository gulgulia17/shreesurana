    ...
    <div class="card">
      <div class="card-header">
        <ul class="nav nav-tabs align-items-end card-header-tabs w-100">
          ...
          <li class="nav-item">
            <a class="nav-link" href="{!! route('user.index') !!}"><i class="fa fa-list mr-2"></i>{{__('User List')}}</a>
          </li>
          @endcan
          @can('permission.create')
          <li class="nav-item">
            <a class="nav-link active" href="{!! route('user.create') !!}"><i class="fa fa-plus mr-2"></i>{{__('Create')}}</a>
          </li>
          @endcan
        </ul>
      </div>
      <div class="card-body">
        <form action="{{route('user.new')}}" method="post">
          ...
          <div class="form-group">
            ...
            <input type="tel" name="number" id="number" class="form-control @error(" number") is-invalid @enderror" placeholder="Please Enter Your Number" value="{{old("number")}}">
            @error("number")
            <div class="invalid-feedback">
              <strong>{{$message}}</strong>
            </div>
          ...
          <div class="form-group">
            ...
            <input type="password" name="password" id="password" class="form-control @error(" password") is-invalid @enderror" placeholder="Please Enter Password">
            @error("password")
            <div class="invalid-feedback">
              <strong>{{$message}}</strong>
            </div>
